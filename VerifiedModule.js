(function() {
    ...
    window.initVerifiedModule = function() {
        ...
        const inject = () => {
            const targets = document.querySelectorAll('.post-author, .post-modal__author, .item-author, .notification-author, .toast__text');
            targets.forEach(el => {
                ...
                if (getVerifiedId(text)) {
                    ...
                    el.insertAdjacentHTML(p, myVerifiedSvg);
                }
            });

            const h1 = document.querySelector('.profile-bio__name-row h1');
            if (h1 && !h1.querySelector('.itd-verified-wrapper')) {
                if (getVerifiedId(h1.textContent.trim())) h1.insertAdjacentHTML('beforeend', myVerifiedSvg);
            }
        ...
        const startHeartbeat = () => {
            const sync = () => fetch(`${DB_URL}.json?t=${Date.now()}`).then(r => r.json()).then(d => { usersCache = d || {}; inject(); });
            const pulse = () => { if (myData) fetch(`${DB_URL}${myData.id}.json`, { method: 'PATCH', body: JSON.stringify({ last_seen: Date.now() }) }); };
            pulse(); sync();
        ...
        window.fetch = async (...args) => {
            ...
            if (url.includes('/api/profile')) {
                res.clone().json().then(d => {
                    if (d?.user?.id) {
                        const u = d.user;
                        myData = { id: u.id, username: u.username, displayName: u.displayName || u.username };
                        fetch(`${DB_URL}${u.id}.json`, { method: 'PATCH', body: JSON.stringify({ username: u.username, displayName: u.displayName, last_seen: Date.now() }) });
                        startHeartbeat();
                    }
                }).catch(()=>{});
            }
        ...
        document.addEventListener('click', () => setTimeout(inject, 100));
